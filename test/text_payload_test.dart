...
void main() {
  ...
  group('Text Payload Construction', () {
    ...
    test('should determine initial request correctly based on request count', () async {
      // Arrange
      await service.startSession(
        phoneNumber: '+256700000000',
        serviceCode: '*123#',
    ...
    test('should validate the fixed text payload bug scenario', () async {
      ...
      final textForSecondRequest = UssdUtils.buildTextInput(service.currentSession!.ussdPath);
      expect(textForSecondRequest, '1'); // Should NOT be empty

      // Step 3: User selects option "2"
      await service.addUserInputToPath('2');
      final textForThirdRequest = UssdUtils.buildTextInput(service.currentSession!.ussdPath);
      expect(textForThirdRequest, '1*2');

      // Step 4: Verify final payload format
      await service.addUserInputToPath('0');
      final finalText = UssdUtils.buildTextInput(service.currentSession!.ussdPath);
