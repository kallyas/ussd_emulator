  ...
  group('SecurityMonitor', () {
    ...
    group('logSecurityEvent', () {
      ...
      test('should determine correct severity levels', () {
        ...
        expect(events[3].severity, SecuritySeverity.critical); // rate_limit_exceeded
        expect(events[2].severity, SecuritySeverity.high);     // suspicious_input_pattern
        expect(events[1].severity, SecuritySeverity.medium);   // unusual_service_code
    ...
    group('detectAnomalousActivity', () {
      ...
      test('should detect suspicious input patterns', () {
        ...
        final events = monitor.getRecentEvents(eventType: 'suspicious_input_pattern');
        expect(events.length, 1);
      ...
      test('should detect unusual session ID length', () {
        final request = UssdRequest(
          sessionId: 'very_very_long_session_id_that_exceeds_normal_length_and_might_indicate_an_attack_or_bug_${'x' * 100}',
          phoneNumber: '+256700000000',
          text: '1',
          serviceCode: '*123#',
        );
      ...
      test('should detect unusual text length', () {
        ...
        final events = monitor.getRecentEvents(eventType: 'unusual_text_length');
        expect(events.length, 1);
      });
      ...
      test('should detect unusual service codes', () {
        ...
        final events = monitor.getRecentEvents(eventType: 'unusual_service_code');
        expect(events.length, 1);
      });
      ...
      test('should track repeated suspicious activity', () {
        ...
        final events = monitor.getRecentEvents(eventType: 'repeated_suspicious_activity');
        expect(events.length, 1);
    ...
    group('getRecentEvents', () {
      test('should filter events by severity', () {
        monitor.logSecurityEvent('critical_event', {}, severity: SecuritySeverity.critical);
        monitor.logSecurityEvent('medium_event', {}, severity: SecuritySeverity.medium);
        monitor.logSecurityEvent('low_event', {}, severity: SecuritySeverity.low);
        
        final criticalEvents = monitor.getRecentEvents(minSeverity: SecuritySeverity.critical);
        expect(criticalEvents.length, 1);
        expect(criticalEvents.first.event, 'critical_event');
        
        final mediumAndAbove = monitor.getRecentEvents(minSeverity: SecuritySeverity.medium);
    ...
    group('getSecurityStats', () {
      test('should return correct statistics', () {
        monitor.logSecurityEvent('event_type_1', {}, severity: SecuritySeverity.high);
        monitor.logSecurityEvent('event_type_1', {}, severity: SecuritySeverity.high);
        monitor.logSecurityEvent('event_type_2', {}, severity: SecuritySeverity.medium);
