...
class SecureUssdApiService extends UssdApiService {
  late final Dio _dio;
  static const String _appSecret = 'ussd_emulator_secret_key'; // Should be from secure storage
  ...
  UssdRequest _sanitizeRequest(UssdRequest request) {
    try {
      final sanitizedSessionId = SecureUssdUtils.validateSessionId(request.sessionId);
      final sanitizedPhoneNumber = SecureUssdUtils.validatePhoneNumber(request.phoneNumber);
      final sanitizedServiceCode = SecureUssdUtils.validateServiceCode(request.serviceCode);
  ...
  String _generateRequestSignature(UssdRequest request, String timestamp) {
    final message = '${request.sessionId}:${request.phoneNumber}:${request.text}:$timestamp';
  ...
  UssdResponse _validateAndParseResponse(Response response) {
    if (response.statusCode == 200) {
      ...
      try {
        ...
        if (responseBody.startsWith('CON ') || responseBody.startsWith('END ')) {
  ...
  String _filterSensitiveData(String logString) {
    ...
    filtered = filtered.replaceAllMapped(
      RegExp(r'\+?\d{10,15}'),
      (match) => '${match.group(0)!.substring(0, 3)}****${match.group(0)!.substring(match.group(0)!.length - 2)}',
    );
    
    // Remove potential passwords or tokens
    filtered = filtered.replaceAllMapped(
      RegExp(r'("password"|"token"|"key"|"secret")\s*:\s*"[^"]*"', caseSensitive: false),
      (match) => '${match.group(1)}":"[REDACTED]"',
    );
    
    // Remove signatures
    filtered = filtered.replaceAllMapped(
      RegExp(r'(signature|auth|authorization):\s*[a-fA-F0-9]{32,}', caseSensitive: false),
      (match) => '${match.group(1)}:[REDACTED]',
    );
