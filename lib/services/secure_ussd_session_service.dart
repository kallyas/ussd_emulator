...
class SecureUssdSessionService extends UssdSessionService {
  ...
  Future<UssdSession> startSession({
    ...
    final validatedPhoneNumber = SecureUssdUtils.validatePhoneNumber(phoneNumber);
    final validatedServiceCode = SecureUssdUtils.validateServiceCode(serviceCode);
    
    // Check rate limits
    if (SecureUssdUtils.checkRateLimit(validatedPhoneNumber)) {
      throw Exception('Rate limit exceeded for phone number: $validatedPhoneNumber');
    }
